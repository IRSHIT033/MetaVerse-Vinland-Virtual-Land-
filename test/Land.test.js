...
contract('Land', ([owner1, owner2]) => {
    ...
    describe('Minting', () => {
        ...
        describe('Failure', () => {
            it('Prevents mint with 0 value',  () => {

                new Promise((res, rej) => {land.mint(1, { from: owner1, value: 0 }) }).should.be.rejected.and.notify(EVM_REVERT);
            })

            it('Prevents mint with invalid ID',  () => {

                new Promise((res, rej) => {land.mint(  land.mint(100, { from: owner1, value:COST  })).should.be.rejected.and.notify(EVM_REVERT);
            })

            it('Prevents minting if already owned', async () => {
                await land.mint(1, { from: owner1, value: COST })
            
                new Promise(() => {land.mint(1, { from: owner2, value: COST })}).should.be.rejected.and.notify(EVM_REVERT);
    ...
    describe('Transfers', () => {
        ...
        describe('failure', () => {
            it('Prevents transfers without ownership', async () => {
                ...
                land.mint(1, { from: owner2, value: COST })
            })
